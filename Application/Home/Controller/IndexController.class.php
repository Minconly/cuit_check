...
class IndexController extends HomeBaseController {
	...
	public function getMenulist(){
        $prefix = "menulist:";
        $_thisAdminInfo = session('accInfo');
        $prefix.=$_thisAdminInfo['role'];
        if($this->getRedis()->get($prefix)) {
            $newArr = json_decode($this->getRedis()->get($prefix));
        }else{
            $lists = array();
            //获取所有的顶级的菜单
            $topmenus = M('Menu')->field('id,name,url,sort,icon')
                ->where(array('parent_id'=>0,'del_flag'=>1))
            ...
            $this->getRedis()->set($prefix,json_encode($newArr));
        }
