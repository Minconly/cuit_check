        ...
        <div class="layui-input-inline" style="">
              <select name="col" id="col" lay-verify="" lay-filter="filter1">
                  <option value="">选择学院</option>
                   <?php if(is_array($collegelist)): $i = 0; $__LIST__ = $collegelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$co): $mod = ($i % 2 );++$i;?><option value="<?php echo ($co['id']); ?>"><?php echo ($co["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
              </select> 
       </div>
      ...
      <fieldset class="layui-elem-field">
        ...
        <div class="layui-field-box">
          <table class="site-table table-hover">
            ...
            <tbody>
          <?php if(is_array($lessionlst)): $i = 0; $__LIST__ = $lessionlst;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
              <td><?php echo ($vo["name"]); ?></td>
              <td><?php echo ($vo["course_name"]); ?></td>
              <td><?php echo ($vo["college_name"]); ?></td>
              <td><?php if(($vo['remarks']) == ""): ?>无
<?php else: ?>
<?php echo ($vo["remarks"]); endif; ?></td>
              <td>
...
<script type="text/javascript">
  var editurl = "<?php echo U('Home/lessionMgr/editlession');?>";
  var listurl = "<?php echo U('Home/LessionMgr/lessionList');?>";
  var deleteurl = "<?php echo U('Home/lessionMgr/deletelession');?>";
  var rooturl = "/";
...
var layer = layui.layer,
  ...
  $('#searchAll').on('click', function(){
    window.location.href="<?php echo U('Home/lessionMgr/lessionList');?>";
  });
  //编辑按钮功能
